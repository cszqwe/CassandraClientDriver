...
public class PopularItem {
    ...
    public PopularItem(Session session) {
        ...
        this.queryCount = session.prepare("select count(OL_O_ID) as numSatisfied from order_line where OL_I_ID=? and OL_W_ID=? and OL_D_ID=? and Ol_O_ID < ? and OL_O_ID >= ? allow filtering;");
    }
    ...
    private void findPopularItemXact(int w_id, int d_id, int orderNum, int numLastOrders) {
        ...
        List<Integer> dedupped = new ArrayList<Integer>(new LinkedHashSet<Integer>(popularItems));
        ...
        for (int k=0; k<dedupped.size(); k++){
            ...
            Row rowResult = result.one();
            if (rowResult != null) {
                long count = rowResult.getLong("numSatisfied");
